...
class VocabularyCreator:
    ...
    def create_vocab(self, minWordFreq, clean_text_mode):
        ...
        for email in emails["dataset"]:
            if email["mail"]["Spam"] == "true":
                spamSubjects.extend(self.clean_text(email["mail"]["Subject"], clean_text_mode))
                spamBodies.extend(self.clean_text(email["mail"]["Body"], clean_text_mode))
            else:
                hamSubjects.extend(self.clean_text(email["mail"]["Subject"], clean_text_mode))
                hamBodies.extend(self.clean_text(email["mail"]["Body"], clean_text_mode))
    ...
    def clean_text(self, text, cleaning_mode):  # pragma: no cover
        return self.cleaning.clean_text(text, clean_text_mode)
