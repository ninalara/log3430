...
class RENEGE:
    ...
    def process_email(self, new_emails, calculation_mode, is_log_est, is_log_combo):
        ...
        for email in emails:
            ...
            if(self.crud.get_user_id(email_adr)) is None:
                return False
            else:
            ...
            for i in range(1, len(groups)):
                for j in range(len(groups[str(i)]['List_of_members'])):
                    if groups[str(i)]['List_of_members'][j] == user_name:
                        sum_trust += groups[i]['Trust']
                        n_groups += 1
            if(sum_trust > 0 and n_groups > 0):
                avg_group_trust = sum_trust / n_groups
            else:
                avg_group_trust = 0

            # difference de jours entre last_seen_msg et first_seen_msg
            user_activity = self.substract_dates(self.crud.get_user_data(user_id, "Date_of_last_seen_message"),
    ...
    @staticmethod
    def substract_dates(last_seen_msg, first_seen_msg):
        user_activity = 0
        # calcul de difference de jours entre le dernier et le premier message
        if isinstance(last_seen_msg, int) and isinstance(first_seen_msg, int):
            last_seen_msg = date.fromtimestamp(round(last_seen_msg/1000))
            first_seen_msg = date.fromtimestamp(round(first_seen_msg/1000))
            user_activity = last_seen_msg - first_seen_msg
            user_activity = int((str(user_activity).split(" "))[0])
        elif isinstance(last_seen_msg, str) and isinstance(first_seen_msg, str):
            last_seen_msg = datetime.datetime.strptime(last_seen_msg, '%Y-%m-%d')
            first_seen_msg = datetime.datetime.strptime(first_seen_msg, '%Y-%m-%d')
        elif last_seen_msg is None or first_seen_msg is None:
            return 0
        user_activity = last_seen_msg - first_seen_msg
        user_activity = int((str(user_activity).split(" "))[0])
